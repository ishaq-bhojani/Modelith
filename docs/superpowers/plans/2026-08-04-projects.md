...
**Interfaces:**
- Consumes: nothing.
- Produces: `ProjectMeta` (re-exported from `src/shared/types.ts` in Task 3) and
...
Run: `npx vitest run tests/unit/project-store.test.ts`
Expected: PASS. `npm run typecheck` will still fail until Task 3 adds `ProjectMeta` — add the type now if you prefer, but it belongs to Task 3's diff.

> If typecheck blocks you, add `ProjectMeta` to `src/shared/types.ts` as part of this task and note it in your report. The interface is in Task 3, Step 3.

- [ ] **Step 5: Commit**

```bash
git add src/main/projects/store.ts tests/unit/project-store.test.ts
...
**Interfaces:**
- Consumes: `ProjectStore` from Task 1; `SessionMeta.projectId` from Task 2.
- Produces:
...
Workspace's confined methods take the root as their **first, required** argument:
...
- [ ] **Step 3: Add `ProjectMeta` and change the Workspace signatures**

Add `ProjectMeta` to `src/shared/types.ts` (interface above).
...
export function registerProjectHandlers(): void {
  ...
  ipcMain.handle(CHANNELS.projectCreate, async () => {
    ...
    // renderer.
    const root = await getWorkspace().pick()
    return root ? projects.list() : projects.list()
  })
